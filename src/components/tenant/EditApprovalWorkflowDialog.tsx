...
interface EditApprovalWorkflowDialogProps {
  open: boolean;
...
export const EditApprovalWorkflowDialog = ({ 
  open, 
  onOpenChange, 
  workflow, 
  onSuccess 
}: EditApprovalWorkflowDialogProps) => {
  ...
  const { data: existingRoles = [] } = useApprovalWorkflowRoles(workflow?.id);
  ...
  useEffect(() => {
    if (workflow) {
      setFormData({
        ...
        minimum_approvers: workflow.minimum_approvers,
        maximum_approvers: workflow.maximum_approvers,
        approval_order: workflow.approval_order,
  ...
  useEffect(() => {
    if (existingRoles.length > 0) {
      setWorkflowRoles(existingRoles.map(role => ({
        role: role.role,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!workflow || !formData.workflow_type_id || !formData.action_type || !formData.table_name) {
      return;
    }
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <Button 
              type="submit" 
              disabled={updateWorkflowMutation.isPending || !formData.workflow_type_id || !formData.action_type || !formData.table_name}
            >
