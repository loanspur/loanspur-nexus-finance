...
INSERT INTO public.savings_products (product_name, description, interest_rate, minimum_balance, tenant_id, is_active)
SELECT 
...
) AS product_data(product_name, description, interest_rate, minimum_balance)
CROSS JOIN (SELECT id as tenant_id FROM tenants LIMIT 1) t
WHERE NOT EXISTS (
  SELECT 1 FROM savings_products sp 
  WHERE sp.product_name = product_data.product_name 
  AND sp.tenant_id = t.tenant_id
);

-- Create loan products if they don't exist
INSERT INTO public.loan_products (product_name, description, interest_rate, max_loan_amount, min_loan_amount, term_months, tenant_id, is_active)
SELECT 
  product_data.product_name,
  product_data.description,
  product_data.interest_rate,
  product_data.max_loan_amount,
  product_data.min_loan_amount,
...
) AS product_data(product_name, description, interest_rate, max_loan_amount, min_loan_amount, term_months)
CROSS JOIN (SELECT id as tenant_id FROM tenants LIMIT 1) t
WHERE NOT EXISTS (
  SELECT 1 FROM loan_products lp 
  WHERE lp.product_name = product_data.product_name 
