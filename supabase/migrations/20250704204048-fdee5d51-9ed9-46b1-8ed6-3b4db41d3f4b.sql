...
USING (
  tenant_id IN (
    SELECT tenant_id FROM public.profiles 
    WHERE user_id = auth.uid() AND role = ANY(ARRAY['tenant_admin'::user_role, 'super_admin'::user_role])
  )
);
...
USING (
  tenant_id IN (
    ...
    EXISTS (
      ...
      AND (awr.role = p.role::text OR awr.custom_role_id = p.custom_role_id)
    )
  )
);
...
WHERE p.role = 'tenant_admin'::user_role
