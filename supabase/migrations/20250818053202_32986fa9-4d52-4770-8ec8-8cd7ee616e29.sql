...
BEGIN
  ...
  FOR loan_record IN 
    ...
    WHERE l.status NOT IN ('closed', 'written_off', 'fully_paid')
  LOOP
...
-- Create index to improve performance of loan harmonization queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_loans_status_active 
ON loans(status, tenant_id) WHERE status NOT IN ('closed', 'written_off', 'fully_paid');

-- Create index for faster loan schedule queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_loan_schedules_loan_payment_status 
ON loan_schedules(loan_id, payment_status, due_date);

-- Create index for faster loan payments queries  
