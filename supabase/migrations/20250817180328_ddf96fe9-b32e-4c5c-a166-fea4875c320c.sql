-- Fix remaining disbursed_on_date references in loan disbursement functions
-- Update the create_loan_disbursement_journal_entry function to use correct field name
  ...
  IF loan_product.accounting_type = 'accrual_periodic' OR loan_product.accounting_type = 'accrual_upfront' THEN
    ...
    IF loan_product.loan_portfolio_account_id IS NOT NULL AND loan_product.fund_source_account_id IS NOT NULL THEN
      ...
      INSERT INTO journal_entries (
        ...
        NEW.tenant_id,
        COALESCE(NEW.disbursement_date, NEW.created_at::date),  -- Fixed: use disbursement_date
