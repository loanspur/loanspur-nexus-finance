...
CREATE TABLE public.loan_products (
  ...
  minimum_principal NUMERIC NOT NULL DEFAULT 0,
  maximum_principal NUMERIC,
  default_principal NUMERIC,
  minimum_loan_term INTEGER NOT NULL DEFAULT 1, -- in months
  maximum_loan_term INTEGER,
  default_loan_term INTEGER,
  interest_rate_type TEXT NOT NULL DEFAULT 'fixed' CHECK (interest_rate_type IN ('fixed', 'variable', 'compound')),
  minimum_interest_rate NUMERIC NOT NULL DEFAULT 0,
  maximum_interest_rate NUMERIC,
  default_interest_rate NUMERIC,
  repayment_frequency TEXT NOT NULL DEFAULT 'monthly' CHECK (repayment_frequency IN ('daily', 'weekly', 'monthly', 'quarterly', 'annually')),
  grace_period_days INTEGER DEFAULT 0,
  allow_partial_payments BOOLEAN NOT NULL DEFAULT true,
  enable_guarantors BOOLEAN NOT NULL DEFAULT false,
...
CREATE POLICY "Users can view their tenant's loan products" 
ON public.loan_products 
FOR SELECT 
USING (tenant_id IN (SELECT tenant_id FROM profiles WHERE user_id = auth.uid()));

CREATE POLICY "Tenant admins can manage loan products" 
ON public.loan_products 
FOR ALL 
USING (
  tenant_id IN (
...
CREATE INDEX idx_loan_products_tenant_id ON public.loan_products(tenant_id);
CREATE INDEX idx_savings_products_tenant_id ON public.savings_products(tenant_id);
CREATE INDEX idx_fee_structures_tenant_id ON public.fee_structures(tenant_id);
CREATE INDEX idx_fee_tiers_fee_structure_id ON public.fee_tiers(fee_structure_id);
CREATE INDEX idx_product_fees_product ON public.product_fees(product_type, product_id);